...
public class Tle
{
    ...
    /// <summary>
    ///     Initialise TLE from OMM (Orbit Mean-Elements Message) data
    /// </summary>
    /// <param name="omm">The OMM data containing orbital elements</param>
    public Tle(OmmData omm)
    {
        Name = omm.ObjectName ?? omm.NoradCatID.ToString();
        Line1 = null;
        Line2 = null;

        NoradNumber = omm.NoradCatID;
        IntDesignator = omm.ObjectID ?? "";
        Epoch = omm.Epoch;
        _meanMotionDt2 = omm.MeanMotionDot / 2.0;
        _meanMotionDdt6 = omm.MeanMotionDDot / 6.0;
        _bstar = omm.BStar;
        _inclination = omm.Inclination;
        _rightAscendingNode = omm.RAOfAscNode;
        _eccentricity = omm.Eccentricity;
        _argumentPerigee = omm.ArgOfPericenter;
        _meanAnomaly = omm.MeanAnomaly;
        _meanMotion = omm.MeanMotion;
        _orbitNumber = omm.RevAtEpoch;
    }
    ...
    /// <summary>
    ///     Parses OMM data from a CSV file and returns a list of Tle objects
    /// </summary>
    /// <param name="path">Path to the CSV file</param>
    /// <returns>List of Tle objects parsed from the OMM CSV data</returns>
    public static List<Tle> ParseOmmCsv(string path)
    {
        var parser = new OmmCsvParser();
        var ommDataList = parser.ParseFile(path);
        return ommDataList.Select(omm => new Tle(omm)).ToList();
    }

    /// <summary>
    ///     Parses OMM data from a JSON file and returns a list of Tle objects
    /// </summary>
    /// <param name="path">Path to the JSON file</param>
    /// <returns>List of Tle objects parsed from the OMM JSON data</returns>
    public static List<Tle> ParseOmmJson(string path)
    {
    ...
    /// <summary>
    ///     Parses OMM data from a KVN file and returns a list of Tle objects
    /// </summary>
    /// <param name="path">Path to the KVN file</param>
    /// <returns>List of Tle objects parsed from the OMM KVN data</returns>
    public static List<Tle> ParseOmmKvn(string path)
    {
        var parser = new OmmKvnParser();
        var ommDataList = parser.ParseFile(path);
        return ommDataList.Select(omm => new Tle(omm)).ToList();
    }

    /// <summary>
    ///     Parses OMM data from an XML file and returns a list of Tle objects
    /// </summary>
    /// <param name="path">Path to the XML file</param>
    /// <returns>List of Tle objects parsed from the OMM XML data</returns>
    public static List<Tle> ParseOmmXml(string path)
    {
        var parser = new OmmXmlParser();
        var ommDataList = parser.ParseFile(path);
        return ommDataList.Select(omm => new Tle(omm)).ToList();
    }

...
public class Satellite
{
    ...
    /// <summary>
    ///     Constructor from OMM (Orbit Mean-Elements Message) data
    /// </summary>
    /// <param name="omm">The OMM data containing orbital elements</param>
    public Satellite(OmmData omm) : this(new Tle(omm))
    {
    }
    ...
    /// <summary>
    ///     Loads a list of satellites from an OMM CSV file
    /// </summary>
    /// <param name="path">Path to the OMM CSV file</param>
    /// <returns>List of Satellite objects</returns>
    public static List<Satellite> LoadFromOmmCsv(string path)
    {
        var tles = Tle.ParseOmmCsv(path);
        return tles.Select(tle => new Satellite(tle)).ToList();
    }

    /// <summary>
    ///     Loads a list of satellites from an OMM JSON file
    /// </summary>
    /// <param name="path">Path to the OMM JSON file</param>
    /// <returns>List of Satellite objects</returns>
    public static List<Satellite> LoadFromOmmJson(string path)
    {
        var tles = Tle.ParseOmmJson(path);
        return tles.Select(tle => new Satellite(tle)).ToList();
    }

    /// <summary>
    ///     Loads a list of satellites from an OMM KVN file
    /// </summary>
    /// <param name="path">Path to the OMM KVN file</param>
    /// <returns>List of Satellite objects</returns>
    public static List<Satellite> LoadFromOmmKvn(string path)
    {
        var tles = Tle.ParseOmmKvn(path);
        return tles.Select(tle => new Satellite(tle)).ToList();
    }

    /// <summary>
    ///     Loads a list of satellites from an OMM XML file
    /// </summary>
    /// <param name="path">Path to the OMM XML file</param>
    /// <returns>List of Satellite objects</returns>
    public static List<Satellite> LoadFromOmmXml(string path)
    {
        var tles = Tle.ParseOmmXml(path);
        return tles.Select(tle => new Satellite(tle)).ToList();
    }
